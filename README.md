...
$ testmaster parse "url1|url2|url3|..." --spider=my_spider -c my_callback --meta='{"x":"y"}' ...
```
If the results generated by such a command are all fine according to your custom rules, the fixtures/tests will be written (assuming you have the middleware enabled and haven't reached the fixtures cap you set). Otherwise, you will get an error message about the failure mode. This is an example of how you can merge the debugging and test-generation processes.
...
```
N.B.: This must be run with the middleware enabled. Otherwise it will just kick off requests that serve no purpose.
...
### Running tests
To run your tests statically, you can use `unittest` regular commands. The validation of the results produced by these tests will observe relevant TESTMASTER settings without the middleware being active.
...
Whereas the above examples will work whether or not the middleware is enabled, the below examples involving the `--dynamic` option require this to succeed.
