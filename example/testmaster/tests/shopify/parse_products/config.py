...
SKIPPED_FIELDS = ["published_at", "created_at", "updated_at", "variant_created_at", "variant_updated_at"]
...
class ItemRules(object):
  def discount_regularity_check(self, item):
      if item["price"] and item["compare_at_price"]:
        assert(float(item["price"]) <= float(item["compare_at_price"])),\
            "Discount regularity check failed"
